...
const PASOS = [
  { emoji: '🌺', gradiente: 'linear-gradient(160deg, #2D6E5E 0%, #4AA08A 100%)', titulo: '¡Bienvenida a UNA!', subtitulo: 'vida y empresa', descripcion: 'La app pensada para ti, que llevas el negocio y la casa al mismo tiempo. En 5 pasos te cuento cómo funciona.', boton: 'Empezar →' },
  { emoji: '💰', gradiente: 'linear-gradient(160deg, #2D6E5E 0%, #5AADA0 100%)', titulo: 'Separa tus finanzas', subtitulo: 'Módulo: Mis Finanzas', descripcion: 'Registra ingresos y gastos del negocio por separado de los del hogar. Siempre sabrás cuánto gana tu negocio y cuánto gastas en casa.', tip: '💡 Registra cada venta el mismo día para no perder el control.', boton: 'Siguiente →' },
  ...
  { emoji: '📅', gradiente: 'linear-gradient(160deg, #C4973A 0%, #E8BF6A 100%)', titulo: 'Nunca olvides nada importante', subtitulo: 'Módulo: Calendario Vital', descripcion: 'Pagos, declaraciones del SAT, citas médicas, fechas escolares y tu cita de bienestar. Todo en un solo lugar con alertas anticipadas.', tip: '💡 Activa los eventos anuales como verificación y Papanicolau para que se repitan solos.', boton: 'Siguiente →' },
  ...
  return (
    <div style={{ ...e.contenedor, background: actual.gradiente }}>
      <div style={e.indicadores}>
        {PASOS.map((_, i) => <div key={i} style={{ ...e.punto, backgroundColor: i === paso ? 'white' : 'rgba(255,255,255,0.35)', width: i === paso ? '24px' : '8px' }} />)}
      </div>
      <div style={e.cuerpo}>
...
const e = {
  ...
  emojiCirculo: { width: '100px', height: '100px', backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '28px' },
  subtitulo: { color: 'rgba(255,255,255,0.75)', fontSize: '12px', letterSpacing: '2px', textTransform: 'uppercase', margin: '0 0 10px 0' },
  titulo: { color: 'white', fontSize: '28px', fontWeight: 'bold', margin: '0 0 16px 0', lineHeight: 1.2 },
  descripcion: { color: 'rgba(255,255,255,0.9)', fontSize: '16px', lineHeight: 1.6, margin: '0 0 24px 0', maxWidth: '320px' },
  tipBox: { backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: '14px', padding: '14px 18px', maxWidth: '320px' },
  ...
  botonSiguiente: { padding: '16px', borderRadius: '16px', border: 'none', background: 'white', color: '#2D6E5E', cursor: 'pointer', fontSize: '16px', fontWeight: 'bold', boxShadow: '0 4px 16px rgba(0,0,0,0.15)' },
  saltar: { background: 'none', border: 'none', color: 'rgba(255,255,255,0.6)', fontSize: '13px', cursor: 'pointer', marginTop: '16px', textAlign: 'center', width: '100%' },
};
