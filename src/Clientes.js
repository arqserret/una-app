...
function Clientes({ onVolver }) {
  ...
  const colores = [P.verde, P.malva, P.dorado, '#5B9BD5', '#7A8C5E'];
    ...
    <div style={e.contenedor}>
      ...
      <div style={{ padding:'16px 20px 0' }}>
        ...
        {clientes.length===0 && !mostrarForm && (
          <div style={e.vacio}><p style={{fontSize:'48px',margin:0}}>👩‍👧</p><p style={{color:P.grisOs,fontWeight:'bold'}}>Aún no tienes clientas</p><p style={{color:P.gris,fontSize:'13px'}}>Toca "+ Nueva" para comenzar</p></div>
        )}
          ...
          return (
            <div key={c.id} style={e.tarjeta}>
              <div style={e.tarjetaTop}>
                <div style={{ ...e.avatar, backgroundColor:colores[idx%colores.length] }}>{c.nombre.charAt(0).toUpperCase()}</div>
                <div style={{ flex:1 }}>
              ...
              <div style={e.acciones}>
                {c.telefono && <>
                  <a href={`tel:${c.telefono}`} style={{ ...e.botonAcc, backgroundColor:P.verdet, color:P.verde }}>📞 Llamar</a>
                  <a href={`https://wa.me/52${c.telefono}`} target="_blank" rel="noreferrer" style={{ ...e.botonAcc, backgroundColor:'#E8F5E9', color:'#388E3C' }}>💬 WhatsApp</a>
                </>}
                <button style={{ ...e.botonAcc, backgroundColor:P.doradot, color:P.dorado, border:'none', cursor:'pointer' }} onClick={()=>editar(c)}>✏️ Editar</button>
                <button style={{ ...e.botonAcc, backgroundColor:P.malvat, color:P.malva, border:'none', cursor:'pointer' }} onClick={()=>eliminar(c.id)}>🗑️</button>
              </div>
            </div>
          );
        })}
...
const e = {
  ...
  buscador:{ flex:1, padding:'12px 14px', borderRadius:'12px', border:'1.5px solid #EAE7E0', fontSize:'14px', backgroundColor:'#FAF9F7' },
  botonNuevo:{ background:'linear-gradient(135deg,#9B7BAE,#C4A0D4)', color:'white', border:'none', padding:'12px 18px', borderRadius:'12px', cursor:'pointer', fontWeight:'bold', fontSize:'14px', whiteSpace:'nowrap' },
  ...
  botonGuardar:{ flex:2, padding:'12px', borderRadius:'12px', border:'none', background:'linear-gradient(135deg,#9B7BAE,#C4A0D4)', color:'white', cursor:'pointer', fontSize:'14px', fontWeight:'bold' },
  ...
  avatar:{ width:'46px', height:'46px', borderRadius:'50%', color:'white', display:'flex', alignItems:'center', justifyContent:'center', fontWeight:'bold', fontSize:'18px', flexShrink:0 },
  clienteNombre:{ fontWeight:'bold', fontSize:'15px', margin:0, color:'#2D2D2D' },
  clienteNotas:{ fontSize:'12px', color:'#8A8A8A', margin:'3px 0 0' },
  citaBadge:{ borderRadius:'10px', padding:'8px 12px', fontSize:'12px', fontWeight:'bold', marginBottom:'10px' },
  acciones:{ display:'flex', gap:'8px', flexWrap:'wrap' },
  botonAcc:{ padding:'8px 12px', borderRadius:'10px', fontSize:'12px', textDecoration:'none', fontWeight:'bold' },
};
